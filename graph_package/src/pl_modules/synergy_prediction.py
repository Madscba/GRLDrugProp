...
class SynergyPrediction(tasks.Task, core.Configurable):
    ...
    def __init__(
        self, model, task=(), criterion="mse", metric=("mae", "rmse"), verbose=0
    ):
    ...
    def evaluate(self, pred, target):
        ...
        for _metric in self.metric:
            if _metric == "mae":
                ...
                score = F.mse_loss(
                    pred * self.std + self.mean, target, reduction="none"
                )
                ...
                for (
                    _pred,
                    _target,
                ) in zip(pred.t(), target.t(), labeled.t()):
