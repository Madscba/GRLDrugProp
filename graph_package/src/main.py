...
from graph_package.src.explainability.pca import save_rescal_embedding
...
def main(config):
    ...
    for k, (train_idx, test_idx) in enumerate(
        ...
        if config.explainability.enable and model_name == "rescal":
            save_rescal_embedding(config, model)
        wandb.finish()
